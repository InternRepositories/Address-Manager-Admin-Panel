...
export class UserUpdateComponent {
  ...
  constructor(
    ...
    this.router.params.subscribe((res) => {
      this.userService.getOne(res['id']).subscribe({
        next: (resp: IApiResponse<User>) => {
          if (resp.status === 200) {
            ...
            console.error('Error getting user', resp.error);
          }
        },
      ...
      .subscribe({
        next: (resp: IApiResponse<User>) => {
          ...
          if (resp.status === 200) {
            ...
            this.snackbar.open('There was an error updating user', undefined, {
              duration: timeout,
            });
        ...
        error: (error: HttpErrorResponse) =>
          console.error('Update One User Error', error.message),
      });
  }
